...
--anims[`GROUP_PISTOL`] = { 'rcmjosh4', 'josh_leadout_cop2', 400, "weapons@pistol@", "aim_2_holster", 450 }
...
function Weapon.Equip(item, data, noWeaponAnim)
	...
	if client.weaponanims then
		...
		--[[if anim == anims[`GROUP_PISTOL`] and  client.hasGroup(shared.police) then
			goto skipAnim 'reaction@intimidation@cop@unarmed'  'intro'
		end]]

		--[[----if anim == anims[`GROUP_PISTOL`] and client.hasGroup(shared.police) then
			local sleep = 600
			print("playanim1")
			Utils.PlayAnimAdvanced(sleep, "reaction@intimidation@cop@unarmed", "intro", coords.x, coords.y, coords.z, 0, 0, GetEntityHeading(playerPed), 8.0, 3.0, sleep*2, 50, 0.1)
			goto skipAnim
		end]]
...
function Weapon.Disarm(currentWeapon, noAnim)
	if currentWeapon?.timer then
		...
		if client.weaponanims and not noAnim then
			...
			--[[if anim == anims[`GROUP_PISTOL`] and client.hasGroup(shared.police) then
				local sleep = 500
				Utils.PlayAnimAdvanced(sleep, "reaction@intimidation@cop@unarmed", "outro", coords.x, coords.y, coords.z, 0, 0, GetEntityHeading(playerPed), 8.0, 3.0, sleep*2, 50, 0.1)
				goto skipAnim
			end]]

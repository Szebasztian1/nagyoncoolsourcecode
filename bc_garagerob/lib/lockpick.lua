...
function LockPick(num)
    ...
    while lockpicking do 
        ...
        for _, status in pairs(locks) do 
            ...
            DrawSprite("MPSafeCracking",lockString,xPos,0.5,0.025,aspectRatio*0.015,0,231,194,81,255)
            xPos = xPos + 0.05
        end 
...
function UpdateLockPicks() 
    if Config.NeedLockPick then 
        ESX.TriggerServerCallback("bc_garagerob:getLockPicks", function(count) 
            lockpickamount = count
            if lockpickamount < 1 then 
                lockpicking = false 
                Config.Notify("Elfogyott a zártörőd!")
            end 
        end)
    end 
end 

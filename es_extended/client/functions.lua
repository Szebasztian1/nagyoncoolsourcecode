...
function ESX.ShowNotification(message, type, length)
    ...
    --end
    TriggerEvent('brutal_notify:SendAlert', "Rendszer", message, (length or 4000), "info")
...
function ESX.Game.SpawnVehicle(vehicle, coords, heading, cb, networked)
    ...
    CreateThread(function()
        ...
        if networked then
            ...
            SetEntityAsMissionEntity(vehicle, true, true)
            -- bc_kocsitorles: legalis spawn jelolese. Ami nem kap ilyet, azt a
            -- szerver-oldali figyelo szal kitorli.
            if vehicle and vehicle ~= 0 then Entity(vehicle).state:set('bc_spawned', true, true) end
        else
            -- bc: lokalis (nem networked) spawn jelzese a kliens oldali figyeloknek
...
--[[
	...
	if DoesEntityExist(vehicle) then
		...
		cpr, cpg, cpb = GetVehicleCustomPrimaryColour(vehicle)
		
		csr, csg, csb = GetVehicleCustomSecondaryColour(vehicle)
		
		ptm1, cm1, pcm1 =  GetVehicleModColor_1(vehicle)
		
		ptm2, cm2 = GetVehicleModColor_2(vehicle)

        local frontoffset = false
        local rearoffset = false
        local frontrot = false
        local rearrot = false

        if GetResourceState("villamos_stancer") == "started" then 
            frontoffset = exports['villamos_stancer']:GetFrontOffset(vehicle)
            rearoffset = exports['villamos_stancer']:GetRearOffset(vehicle)
            frontrot = exports['villamos_stancer']:GetFrontRot(vehicle)
            rearrot = exports['villamos_stancer']:GetRearRot(vehicle)
        end 
		...
		return {
			...
			fuelLevel         = DecorGetFloat(vehicle, "FUEL_LEVEL") or 30.0,
            ...
            modLiveryB        = GetVehicleMod(vehicle, 48),
            modfrontoffset = frontoffset,
            modrearoffset = rearoffset,
            modfrontrot = frontrot,
            modrearrot = rearrot,
		}
	else
		return
...
ESX.Game.SetVehicleProperties = function(vehicle, props)
	if DoesEntityExist(vehicle) then
		...
		if props.fuelLevel then 
            ...
            DecorSetFloat(vehicle, "FUEL_LEVEL", props.fuelLevel + 0.0) 
        end
		if props.dirtLevel then SetVehicleDirtLevel(vehicle, props.dirtLevel + 0.0) end
		if props.color1 then SetVehicleColours(vehicle, props.color1, colorSecondary) end
		if props.color2 then SetVehicleColours(vehicle, props.color1 or colorPrimary, props.color2) end
		if props.customPrimary then SetVehicleCustomPrimaryColour(vehicle, props.customPrimary.r, props.customPrimary.g, props.customPrimary.b) end
		if props.customSecondary then SetVehicleCustomSecondaryColour(vehicle, props.customSecondary.r, props.customSecondary.g, props.customSecondary.b) end
		if props.colorMod1 then SetVehicleModColor_1(vehicle, props.colorMod1.pT, props.colorMod1.c, props.colorMod1.pC) end
		if props.colorMod2 then SetVehicleModColor_2(vehicle, props.colorMod2.pT, props.colorMod2.c) end
        ...
        if GetResourceState("villamos_stancer") == "started" then 
            if props.modfrontoffset then 
                exports['villamos_stancer']:AddFrontOffset(vehicle, props.modfrontoffset)
            end 
            if props.modrearoffset then 
                exports['villamos_stancer']:AddRearOffset(vehicle, props.modrearoffset)
            end 
            if props.modfrontrot then 
                exports['villamos_stancer']:AddFrontRot(vehicle, props.modfrontrot)
            end 
            if props.modrearrot then 
                exports['villamos_stancer']:AddRearRot(vehicle, props.modrearrot)
            end
        end 
        ...
        if props.modLiveryB then SetVehicleMod(vehicle, 48, props.modLiveryB, false) end
...
function ESX.Game.GetVehicleProperties(vehicle)
    ...
    if GetResourceState("rcore_fuel") == "started" then 
		rcfuel = exports["rcore_fuel"]:GetFuel(vehicle)
	end 
    ...
    return {
        ...
        fuelLevel = (rcfuel or ESX.Math.Round(GetVehicleFuelLevel(vehicle), 1)),
        dirtLevel = ESX.Math.Round(GetVehicleDirtLevel(vehicle), 1),
        color1 = colorPrimary,
        color2 = colorSecondary,
...
function ESX.Game.SetVehicleProperties(vehicle, props)
    ...
    if props.fuelLevel ~= nil then
        SetVehicleFuelLevel(vehicle, props.fuelLevel + 0.0)
        TriggerEvent("ox_fuel:setfuel", vehicle, props.fuelLevel + 0.0)
        DecorSetFloat(vehicle, "FUEL_LEVEL", props.fuelLevel + 0.0) 
        if GetResourceState("rcore_fuel") == "started" then 
			exports["rcore_fuel"]:SetFuel(vehicle, props.fuelLevel + 0.0)
		end 
    end

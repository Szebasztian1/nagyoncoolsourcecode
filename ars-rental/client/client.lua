            ...
            exports.ox_target:addLocalEntity(ped, {
                {
                    ...
                    onSelect = function(entity)
                        if self.rental and self.rental.job and ESX.PlayerData.job.name ~= self.rental.job then 
                            return TriggerEvent("esx:showNotification", "Ehhez nincs jogosultságod!")
                        end 
...
local function onNearby(self)
    if not Config.Target then
        DrawMarker(21, self.coords.x, self.coords.y, self.coords.z - 1, 0.0, 0.0, 0.0, 0.0, 180.0, 0.0, 0.6, 0.6, 0.6, 58,
            77, 87, 200, true, true, 2, nil, nil, false)
    end

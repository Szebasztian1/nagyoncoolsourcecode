...
SD.Locale.T = function(key, replacements)
    ...
    if lstr and replacements then
        -- Single pass with a function replacement: values are inserted literally,
        -- so a '%' in user data (player names, item labels) cannot break gsub.
        lstr = lstr:gsub('{(.-)}', function(k)
            local v = replacements[k]
            return v ~= nil and tostring(v) or nil  -- nil keeps the original {key}
        end)
    end

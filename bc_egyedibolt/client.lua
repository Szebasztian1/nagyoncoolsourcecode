...
CreateThread(function()
    ...
    ESX.TriggerServerCallback("bc_egyediautoker:getprices", function(data)
        vehprices = data
    end)
                ...
                if data.sellcoords and IsPedInAnyVehicle(ped) then
                    ...
                    if sdis < 20 then 
                        ...
                        if sdis < 2.0 then 
                            ...
                            if IsControlJustReleased(0, 38) then
                                TriggerServerEvent("bc_egyediautoker:sellVehicle", shop)
                                Wait(2000)
                            end
                        end
                    end 
...
function OpenShop(shop)
    ...
    openedShop = shop
    ESX.TriggerServerCallback("bc_egyediautoker:openShop", function(cars, money) 
        if not cars then 
            openedShop = false 
            return 
        end 
...
RegisterNUICallback('buy', function(data, cb)
    ...
    TriggerServerEvent('bc_egyediautoker:buyVehicle', openedShop, data.model, 'money')
    CloseShop()
    cb(1)
end)
...
RegisterNUICallback('buybank', function(data, cb)
    ...
    TriggerServerEvent('bc_egyediautoker:buyVehicle', openedShop, data.model, 'bank')
    CloseShop()
    cb(1)
end)
...
RegisterNUICallback('buyfaction', function(data, cb)
    ...
    TriggerServerEvent('bc_egyediautoker:buyVehicleFaction', openedShop, data.model)
    CloseShop()
    cb(1)
end)
...
RegisterNUICallback('test', function(data, cb)
    ...
    ESX.TriggerServerCallback("bc_egyediautoker:getVehDataFromPlate", function(can)
        if can then 
...
RegisterNetEvent('bc_egyediautoker:spawnCar', function(coords, plate)
    ...
    ESX.TriggerServerCallback("bc_egyediautoker:getVehDataFromPlate", function(can)
        ...
        if vehicle and vehicle ~= 0 and NetworkGetEntityIsNetworked(vehicle) then Entity(vehicle).state:set('bc_spawned', true, true) end
        ESX.Game.SetVehicleProperties(vehicle, can)
        exports["gs_eventprotect"]:GS_TriggerServerEvent("bc_egyediautoker:carspawned", NetworkGetNetworkIdFromEntity(vehicle))
...
ESX.RegisterClientCallback("bc_egyediautoker:confirmSell", function(cb, plate, price)
    selling = true 

...
RegisterKeyMapping('+drone_descend', 'Drón: Ereszkedés (E tartva)', 'keyboard', 'e')
...
RegisterKeyMapping('+drone_light', 'Drón: Reflektor kapcsoló', 'keyboard', 'l')
...
function DroneControl.Start(droneEntity, startPos)
    ...
    -- Show control hint
    lib.notify({ type = 'inform', title = 'Drón Irányítás', description =
    'W/S/A/D = Mozgás | Q = Fel | E = Le | SPACE = Lecsatlakozás' })
    ...
    CreateThread(function()
        while controlling do
            ...
            if dist > Config.ControlMaxRange then
                lib.notify({ type = 'error', title = 'Drón', description = 'Jel elveszett — drón visszatér.' })
            ...
            camPitch = math.max(-89.0,
                math.min(89.0, camPitch + GetDisabledControlNormal(0, 2) * -Config.ControlPitchSensitivity))
...
function DroneControl.Stop(droneEntity)
    ...
    lib.notify({ type = 'success', title = 'Drón', description = 'Lecsatlakozva a drónról.' })
end

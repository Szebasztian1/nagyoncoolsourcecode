...
local function stateBagWrapper(keyFilter, cb)
    return AddStateBagChangeHandler(keyFilter, '', function(bagName, _, value, _, replicated)
        ...
        end, 'Timeout while waiting for entity to exist', 10000)
...
-- [bc_radio] This key (Q) is shared with bc_radio, where HOLDING it opens the
-- radio panel. So the lights toggle on RELEASE and only for a short tap; a long
-- hold belongs to the radio and is ignored here. The threshold is read from
-- bc_radio so the two can never drift apart.
local radioHoldMs
local function radioHoldTime()
    if radioHoldMs then return radioHoldMs end

    radioHoldMs = 600
    if GetResourceState('bc_radio') == 'started' then
        local ok, value = pcall(function() return exports.bc_radio:GetHoldTime() end)
        if ok and tonumber(value) then radioHoldMs = tonumber(value) end
    end

    return radioHoldMs
end

local lightsPressedAt = 0

local policeLights = lib.addKeybind({
    name = 'policeLights',
    description = 'Tap this button to use your siren (hold = radio)',
    defaultKey = config.controls.policeLights,
    onPressed = function()
        lightsPressedAt = GetGameTimer()
    end,
    onReleased = function()
        -- long press = bc_radio is opening, leave the lights alone
        if GetGameTimer() - lightsPressedAt >= radioHoldTime() then return end

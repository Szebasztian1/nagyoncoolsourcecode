...
function lib.progressBar(data)
    ...
    if not interruptProgress(data) then
        if GetResourceState("new_progressbar") == "started" then 
            local p = promise.new()
            local dismov = false
            local discarmov = false
            local dismous = false
            local discom = false
            ...
            exports['new_progressbar']:Progress({
                name = "ox_lib",
                duration = data.duration or 2000,
                label = data.label or "",
                useWhileDead = (data.useWhileDead or true),
                canCancel = (data.canCancel or false),
                controlDisables = {
                    disableMovement = dismov,
                    disableCarMovement = discarmov,
                    disableMouse = dismous,
                    disableCombat = discom,
                },
            }, function(cancelled)

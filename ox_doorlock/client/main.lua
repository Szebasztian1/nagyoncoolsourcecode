...
lib.callback('ox_doorlock:getDoors', false, function(data)
	...
	while true do
		...
		for _, door in pairs(doors) do
			...
			local dx, dy = coords.x - dc.x, coords.y - dc.y
			local flat = dx * dx + dy * dy

			-- This walks every door in the database twice a second, so a cheap squared
			-- 2D reject runs before the full vector distance. Anything further than 80m
			-- horizontally is further than that in 3D too, so the branches below behave
			-- identically; 100 is a sentinel that fails every "< 80" / "< 20" test.
			if flat < nearest then nearest = flat end

			if flat > 6400 then
				door.distance = 100
			else
				door.distance = #(coords - dc)
			end
		...
		-- With no door inside 150m the player cannot reach one before the next pass
		-- (a car covers ~80m in that time), so the full rate buys nothing out here.
		Wait(nearest > 22500 and 1500 or 500)
	end
end)

...
-- Purely local: teleporting yourself needs no server round trip
RegisterNUICallback("teleport-to-marker", function(data, cb)
    ...
    teleportToCoords(vector3(x + .0, y + .0, z + .0))
end)
...
RegisterNUICallback("duplicate-job", function(data, cb)
    ESX.TriggerServerCallback('esx_job_creator:duplicateJob', function(result)
        cb(result)
    end, data.oldJobName, data.newJobName, data.newJobLabel)
end)
...
RegisterNUICallback("get-stash-capacity", function(data, cb)
    ESX.TriggerServerCallback('esx_job_creator:stash:getCapacity', function(info)
        cb(info or {})
    end, data.markerId)
end)

RegisterNUICallback("set-stash-capacity", function(data, cb)
    ESX.TriggerServerCallback('esx_job_creator:stash:setCapacity', function(result)
        cb(result)
    end, data.markerId, data.maxWeightKg)
end)

...
local function checkAllowedActions(cb)
    ESX.TriggerServerCallback('esx_job_creator:checkAllowedActions', function(data)
        ...
        TriggerEvent("bc_carry:setpermissions", data)
...
local function openActionsMenu()
    if (canDoAnyAction and (config.canUseActionsMenuWhileOffDuty or isOnDuty)) then
        ...
        if (canHandcuff) then
            ...
            table.insert(elements, { label = "Csomagtartóba tuszkolás", value = "putintrunk" })
            table.insert(elements, { label = "Kivétel a csomagtartóból", value = "takefromtrunk" })
        end
        ...
        if (canCheckVehicleOwner) then
            table.insert(elements,
                { label = getLocalizedText('actions:check_vehicle_owner'), value = "checkvehicleowner" })
        end
        ...
        if (ESX.GetPlayerData().job.name == "ambulance") then 
             table.insert(elements,
                { label = "Mentős menü", value = "ambmenu" })
        end 
        ...
        ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'actions_menu', {
            ...
            function(data, menu)
                ...
                elseif (action == "putintrunk") then
                    TriggerEvent('esx_job_creator:actions:putInTrunk')
                elseif (action == "takefromtrunk") then
                    TriggerEvent('esx_job_creator:actions:takeFromTrunk')
                ...
                elseif (action == "checklicenses") then
                    checkLicenses()
                elseif (action == "ambmenu") then
                    TriggerEvent("bc_amb:emenu")
                end
            end,
            function(data, menu)
                menu.close()

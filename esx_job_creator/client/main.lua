        ...
        Citizen.CreateThread(function()
            ...
            while distance < config.markerDistance and activeMarkers[markerId] do
                ...
                if (not config.use3Dtext) then
                    ...
                    ESX.Game.Utils.DrawText3D(vector3(marker.coords.x, marker.coords.y, marker.coords.z + 1.0),
                        marker.label, config.textSize, config.textFont)
                end
...
local function spawnPed(markerData)
    ...
    Citizen.CreateThread(function()
        ...
        local ped = CreatePed(1, markerData.ped.model, markerData.coords.x, markerData.coords.y, markerData.coords.z,
            markerData.ped.heading + .0, false, false)
    ...
    Citizen.CreateThread(function()
        while true do
            ...
            for k, marker in pairs(markers) do
                if marker.type ~= "garage_buyable" then
                    if (marker.coords and type(marker.coords) ~= "table") then
                        marker.coords = json.decode(marker.coords)
                    end

                    local distance = GetDistanceBetweenCoords(plyPos, marker.coords.x, marker.coords.y, marker.coords.z,
                        true)
    ...
    ESX.TriggerServerCallback('esx_job_creator:getMarkers', function(markersData)
        ...
        for markerId, markerData in pairs(markersData) do
            ...
            if (markerData.blip.spriteId and isThisMarkerEnabled) then
                ...
                SetBlipScale(blip, (markerData.blip.scale or 1.0) + .0)
...
AddEventHandler('esx:playerLoaded', function(playerData)
    print("^2[esx_job_creator] esx:playerLoaded fired^7")
    getMarkers(function(markersData)
        local count = 0
        for _ in pairs(markersData) do count = count + 1 end
        print("^2[esx_job_creator] getMarkers returned " .. count .. " markers^7")
        markers = markersData
        activateMarkers()
        print("^2[esx_job_creator] activateMarkers called^7")
    end)
end)
...
local function playAnimation(animationData)
    local plyPed = PlayerPedId()
    local mcd = GetEntityCoords(plyPed)
    ...
    if (animationData.type == "scenario") then
        ...
        ClearAreaOfObjects(plyCoords, 2.0, 0)

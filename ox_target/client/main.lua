...
-- Halottan a kórházon kívül tiltott a targetelés; a jelzőt az esx_ambulancejob állítja
local deadTargetNotifyAt = 0
...
local function startTargeting()
    ...
    if LocalPlayer.state.deadTargetBlocked then
        local now = GetGameTimer()

        if now > deadTargetNotifyAt then
            deadTargetNotifyAt = now + 3000
            TriggerEvent('esx:showNotification', 'Halottan csak a kórházban használhatod az interakciót!')
        end

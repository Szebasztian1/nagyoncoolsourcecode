...
for zoneName, zoneData in pairs(Config.HuntingZones) do
    ...
    -- a zona blipjei a jatekos spawnja utan jonnek letre, lasd utils.waitForSpawnBeforeBlips
    CreateThread(function()
        utils.waitForSpawnBeforeBlips()
        ...
        if zoneData.blip.enable then
            ...
            MunkaBlip('vadaszat', utils.createBlip(zoneData.blip))
...
local function placeBait()
    ...
    if prop and prop ~= 0 and NetworkGetEntityIsNetworked(prop) then Entity(prop).state:set('bc_spawned', true, true) end
    ...
    CreateThread(function()
        while DoesEntityExist(prop) do
            ...
            if dist <= 55 then
                ...
                utils.drawText3D(vector3(coords.x, coords.y, coords.z - 0.5), "~o~ Csali", 1, 0)
            end

...
if not Config.UseDeferrals then
	...
	RegisterNetEvent('esx_identity:showreRegisterIdentity')
	AddEventHandler('esx_identity:showreRegisterIdentity', function()
		while not loadingScreenFinished do
		Citizen.Wait(100)
	end
...
RegisterNetEvent("esx_identity:myReferal", function(code)
	lib.alertDialog({
        header = 'Meghívókód',
        content = 'A meghívókódod: '..code,
        centered = true,
    })
end)
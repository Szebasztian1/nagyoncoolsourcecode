...
function sanitizeKeyContent(raw) {
    // Force string, normalize unicode to reduce tricks like full-width chars
    const s = String(raw ?? "").normalize("NFKC").trim();

    // Allow only: a-z A-Z 0-9 _ - .
    // (no slashes, no spaces, no quotes, no < >, etc.)
...
function CreateKey(keyType, keyContent) {
    ...
    if (keyType === 0) o.className = "key keySmall";
    else if (keyType === 1) o.className = "key keyBig";
    else if (keyType === 2) o.className = "keyCustom";

    const safe = sanitizeKeyContent(keyContent);

    if (safe && safe.toLowerCase().endsWith(".png")) {

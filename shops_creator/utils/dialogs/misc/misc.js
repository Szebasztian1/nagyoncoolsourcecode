...
function isThereAnyErrorInForm(event) {
    ...
    if (!event.target.checkValidity()) {
        ...
        swal(getLocalizedText("menu:error"), getLocalizedText("menu:invalid_fields").replace("%s", invalidInputs.length), "error");
...
function translateEverything() {
	...
	// TomSelect renders its own copy of the <option>s, so it has to re-read them
	$("[data-select]").each(function() {
		if(this.tomselect && this.tomselect.sync) this.tomselect.sync();
	})
}
...
async function refreshTranslations(locale) {
	...
	// sweetalert's shorthand form falls back to an english "OK" button
	if(typeof swal != "undefined" && swal.setDefaults) {
		swal.setDefaults({ button: getLocalizedText("menu:ok") });
	}
...
async function showServerResponse(response) {
	...
	if(response === true) {
		swal(getLocalizedText("menu:success"), "", "success");

	} else {
		swal(getLocalizedText("menu:error"), response || "", "error");
	}
}
...
async function confirmDeletion(message) {
	const wantsToDelete = await swal({
		...
		text: message,
		icon: "warning",
		buttons: [ getLocalizedText("menu:cancel"), getLocalizedText("menu:confirm") ],
		dangerMode: true,
	});
